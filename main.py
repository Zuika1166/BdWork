...
tabs.add(root, text ='Продать')
tabs.add(root2, text ='Запас')
...
def GenerateBill():
    ...
    if p1quantity.get()==0 and p2quantity.get()==0 and p3quantity.get()==0 and p4quantity.get()==0:
        ...
        if p1quantity.get()!=0:
            ...
            print(price)
            billarea.insert(END,f"\n{dateE.get()}\t Продукт-1 \t{p1price.get()}\t {p1quantity.get()}\t {price}")

            sql = '''
            INSERT INTO Sellings VALUES 
            (?, ?, ?, ?,?)
            '''
            cur.execute(sql,(dateE.get(),'Продукт-1',p1price.get(),p1quantity.get(),price))
            connectObj.commit() 

        if p2quantity.get()!=0:
            price2=(p2quantity.get()*p2price.get())
            print(price2)
            billarea.insert(END,f"\n{dateE.get()}\t Продукт-2 \t{p2price.get()}\t {p2quantity.get()}\t {price2}")

            sql = '''
            INSERT INTO Sellings VALUES 
            (?, ?, ?, ?,?)
            '''
            print(dateE.get(),'Продукт-1',p2price.get(),p2quantity.get(),price2)
            cur.execute(sql,(dateE.get(),'Продукт-1',p2price.get(),p2quantity.get(),price2))
            connectObj.commit() 

        if p3quantity.get()!=0:
            price3=p3quantity.get()*p1price.get()
            print(price3)
            billarea.insert(END,f"\n{dateE.get()}\tПродукт-3 \t{p3price.get()}\t {p3quantity.get()}\t {price3}")

            sql = '''
            INSERT INTO Sellings VALUES 
            (?, ?, ?, ?,?)
            '''
            cur.execute(sql,(dateE.get(),'Продукт-3',p3price.get(),p3quantity.get(),price3))
            connectObj.commit() 

        if p4quantity.get()!=0:
            price4=p4quantity.get()*p1price.get()
            billarea.insert(END,f"\n{dateE.get()}\tПродукт-4 \t{p4price.get()}\t {p4quantity.get()}\t {price4}")

            sql = '''
            INSERT INTO Sellings VALUES 
            (?, ?, ?, ?,?)
            '''
            cur.execute(sql,(dateE.get(),'Продукт-4',p4price.get(),p4quantity.get(),price4))
            connectObj.commit() 
...
def view():
    ...
    rows=cur.fetchall()
    viewarea.insert(END,f"Дата\t Продукт\t  Цена 1\t  Кол-во\t  Цена\n")
...
dateL=Label(root,text="Дата",bg="DodgerBlue2",width=12,font=('arial',15,'bold'))
...
l=Label(root, text="Продукт",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=1,column=0,padx=7,pady=7)

l=Label(root, text="Цена",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=1,column=1,padx=7,pady=7)

l=Label(root, text="Кол-во",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=1,column=2,padx=7,pady=7)

#----product 1----------------------------------------------------
p1name=StringVar()
p1name.set('Продукт 1')
...
viewbtn=Button(root,command=view,text="Посмотреть все настройки",
font=('arial',15,'bold'),bg="DodgerBlue2",width=20 )
...
def viewStock():
    ...
    rows=cur.fetchall()
    viewarea2.insert(END,f"Дата \tПродукт\t  Цена\t  Кол-во\t \n")
...
dateL=Label(root2,text="Дата",bg="DodgerBlue2",width=12,font=('arial',15,'bold'))
dateL.grid(row=0,column=0,padx=7,pady=7)

dateE2=DateEntry(root2,width=12,font=('arial',15,'bold'))
dateE2.grid(row=0,column=1,padx=7,pady=7)

l=Label(root2, text="Продукт",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=1,column=0,padx=7,pady=7)

l=Label(root2, text="Цена",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=2,column=0,padx=7,pady=7)

l=Label(root2, text="Кол-во",font=('arial',15,'bold'),bg="DodgerBlue2",width=12)
l.grid(row=3,column=0,padx=7,pady=7)
...
addbtn=Button(root2,command=addStock,text="Добавить",
font=('arial',15,'bold'),bg="DodgerBlue2",width=20)
...
viewbtn2=Button(root2,command=viewStock,text="Посмотреть акции",
font=('arial',15,'bold'),bg="DodgerBlue2",width=20 )
